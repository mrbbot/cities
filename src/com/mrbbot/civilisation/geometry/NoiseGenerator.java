...
/**
 * Class containing static methods for generating random noise to be used by the terrain generator. Code used is from
 * this YouTube video: https://youtu.be/qChQrNWU9Xw
 */
public class NoiseGenerator {
  ...
  /**
   * Gets some random noise for the specified coordinate. This function will always return the same value for the same
   * coordinate in the same program execution.
   *
  ...
  private static double getNoise(int x, int y) {
    // Set the seed of the random generator as a constant plus some multiple of the x and y coordinates. This ensures
    // the same coordinates generate the same noise.
  ...
  /**
   * Gets a smoothed version of the random noise for the specified coordinates. Corners, edges, and the center are
   * all taken into account with different proportions.
   *
   * @param x x-coordinate of noise to get
   * @param y y-coordinate of noise to get
   * @return random noise in the range -1 < noise < 1
  ...
  private static double getSmoothNoise(int x, int y) {
    return
      ((getNoise(x - 1, y - 1) + getNoise(x + 1, y - 1) + getNoise(x + 1, y + 1) + getNoise(x - 1, y + 1)) / 16.0)
  ...
  /**
   * Gets an interpolated version of the random noise. This function unlike the others in this class takes doubles
   * for the coordinates allowing for decimal positions to be used.
   *
   * @param x x-coordinate of noise to get
   * @param y y-coordinate of noise to get
   * @return random noise in the range -1 < noise < 1
  ...
  public static double getInterpolatedNoise(double x, double y) {
    ...
    // Gets the floors and ceilings of the x and y coordinates. These are the vertices of the quadrant to get the
    // noise from,.
    ...
    // Get noise for the position in the quadrant proportional to the fractional coordinate components
